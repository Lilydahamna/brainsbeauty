  ...
  const renderTabContent = (index) => {
    if (index === 0) {
      if (product.category == "sun") {
        return (
          <div>
            {Object.entries(product.uv_filters).map(
        ...
        return (
          <div>
            {product.active_ingredient.map((ingredient) => (
              <div key={ingredient}>
                <strong>{ingredient}:</strong> {actives[ingredient]}
              </div>
            ))}
          </div>
        );
      ...
      return <div>{product.ingredient_list}</div>;
    } else if (index === 2) {
      return <div>{product.directions}</div>;
    }
  };
